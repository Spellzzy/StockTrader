...
def _do_watch(codes_str: str, interval: int, alert_only: bool, sort_by: str = "chg"):
    """看盘模式内部实现（含轮次间差异对比）"""
    from app.services.alert_service import AlertService
    from app.services.watchlist_service import WatchlistService
    from app.services.market_service import MarketService
            ...
            try:
                ...
                if not alert_only:
                    ...
                    for code in stock_codes:
                        ...
                        if not vr and code in _vol_avg5 and _vol_avg5[code] > 0 and _elapsed_min > 0:
                            cur_vol = q.get("volume", 0)
                            expected_vol = _vol_avg5[code] * (_elapsed_min / TOTAL_MINUTES)
                            vr = cur_vol / expected_vol if expected_vol > 0 else 0
                        _vr_cache[code] = vr
...
def quick_watch(
    ...
    _do_watch(codes, interval, alert_only, sort_by)
